...
window.onload = function() {
	document.getElementById("all").addEventListener("click", showReims);
	document.getElementById("p1").addEventListener("click", pendingReims);
	document.getElementById("a1").addEventListener("click", approvedReims);
	document.getElementById("d1").addEventListener("click", deniedReims);
}
function deniedReims() {
	...
	xhr.open('POST', 'http://localhost:8081/Project1-mk/deniedreimserv');
	xhr.send();
	//e.preventDefault();
}
function approvedReims() {
	const xhr = new XMLHttpRequest();
	//xhr.responseType = 'text';
	xhr.responseType = 'json';
	...
	xhr.open('POST', 'http://localhost:8081/Project1-mk/approvedreimserv');
	xhr.send();
	//e.preventDefault();
}

function pendingReims() {
	const xhr = new XMLHttpRequest();
	//xhr.responseType = 'text';
	xhr.responseType = 'json';
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			//JSON.parse(rJSON);
			//console.log('response');
			PDOMManipulation(rJSON);
		}
	};

	xhr.open('POST', 'http://localhost:8081/Project1-mk/pendingreimserv');
	xhr.send();
	//e.preventDefault();
}
...
function showReims() {
	...
	xhr.open('POST', 'http://localhost:8081/Project1-mk/managerreimserv');
	xhr.send();
	//e.preventDefault();
}
...
function PDOMManipulation(reimJSON) {
	console.log(reimJSON)
	var table = document.createElement("table"), row, cellAmount, cellAuthor, cellDescription, cellReimId, cellStatus, cellReimType, cellResolved, cellResolver, cellSubmitted;
	document.getElementById("table1").appendChild(table);
	var rId, resolver, amount, submitted, descrip, status, rType, resolved, author;
